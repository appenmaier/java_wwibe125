# Komponententests (Unit Tests) und Mockito

**Abbildungen**

<img width="2003" height="1104" alt="image" src="https://github.com/user-attachments/assets/2512ed1e-7843-4d86-8e95-c12e55b13d87" />

**Links**

- [Doku: Komponententests (Unit Tests)](https://jappuccini.github.io/java-docs/production/documentation/unit-tests)
- [Doku: Mockito](https://jappuccini.github.io/java-docs/production/documentation/mockito)
- [Übungsaufgaben: Komponententests (Unit Tests)](https://jappuccini.github.io/java-docs/production/exercises/unit-tests/)
- [Demo: Komponententests (Unit Tests)](../src/test/java/model/CrateTest.java)
