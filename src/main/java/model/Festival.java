...
public record Festival(String name, Map<Performance, Stage> schedule) { // 1
   ...
   public Optional<Stage> getStageByBandName(String bandName) {
      ...
      return schedule.entrySet() // 1
            .stream() // 0,5
            .filter(entry -> entry.getKey().getBandName().equals(bandName)) // 1,5
            .map(entry -> entry.getValue()) // .map(Entry::getValue) // 1
            .findAny(); // 1,5
   } // 5,5
   ...
   public List<Performance> getPerformancesByGenre(MusicGenre genre) {
      ...
      return schedule.keySet() // 1
            .stream() // 0,5
            .filter(p -> p.getGenre().equals(genre)) // 1
            .sorted() // 0,5
            .toList(); // 1
   } // 4,5
